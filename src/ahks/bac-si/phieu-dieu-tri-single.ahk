...
if WinExist("FPT") {
    ...
    loop {
        ...
        if (FindImage(IMAGE_PATH . "het-phieu-1.png", 30) || FindImage(IMAGE_PATH . "het-phieu-2.png", 30) ||
        CheckDateSmaller(DATE, RESULT.firstItem)) {
            Sleep 300
...
TimPhieuDieuTri(x1, y1, x2, y2, date) {
    ...
    for Line in splitOutput {
        ...
        if (StrCompare(Fields[12], date) = 0) {
            ...
            if (A_Index + 2 >= splitOutput.Length) {
                result.foundList.Push({ x: X / Multiple, y: Y / Multiple })
            } else {
                NextTwoItem := StrSplit(splitOutput[A_Index + 2], "`t")[12]
                if (NextTwoItem != "(VTYT)" && NextTwoItem != "(VITYT)" && NextTwoItem != "(VIYT)") {
                    result.foundList.Push({ x: X / Multiple, y: Y / Multiple })
                }
            }
        }
